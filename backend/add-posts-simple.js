    ...
    for (let i = 0; i < samplePosts.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(`❌ Lỗi khi thêm bài viết "${postData.title}":`, errorData.message || 'Lỗi không xác định');
        }
      } catch (error) {
        console.log(`❌ Lỗi khi thêm bài viết "${postData.title}":`, error.message);
      }
