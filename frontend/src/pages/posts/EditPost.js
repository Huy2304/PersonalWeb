...
const EditPost = ({ post: propPost, onPostUpdated, onCancel }) => {
  const { user } = useAuth();
  const { id } = useParams();
  const navigate = useNavigate();
  const [post, setPost] = useState(propPost); // Local state for post
  ...
  useEffect(() => {
    // If post provided via props, use it
    if (propPost) {
      setPost(propPost);
    } else if (id) {
      // Fetch post if ID provided in URL
